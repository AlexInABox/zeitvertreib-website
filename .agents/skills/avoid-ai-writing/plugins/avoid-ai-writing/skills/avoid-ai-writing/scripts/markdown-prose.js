...
function inlineLinkEnds(s) {
  ...
  const bare = new Int32Array(n + 1),
    space = new Int32Array(n + 1);
...
function markdownProse(text) {
  ...
  lines.forEach((line, i) => {
    ...
    const quote = fence
      ? quoteDepth
        ? b.match(new RegExp(`^(?: {0,3}>[ \\t]?){${quoteDepth}}`))
        : null
      : b.match(/^(?: {0,3}>[ \t]?)+/);
  ...
  for (let i = 0; i < s.length; i += 1) {
    ...
    if (s[i] === '`') {
      ...
      while ((end = closeTicks.exec(s)) !== null && end.index < nextBlock[i]) {
        ...
        protect(i, closeTicks.lastIndex);
        i = closeTicks.lastIndex - 1;
        found = true;
        break;
      }
    ...
    if (s[i] === '<') {
      ...
      for (const re of html) {
        ...
        protect(i, re.lastIndex, re === html[0]);
        i = re.lastIndex - 1;
        matched = true;
        break;
      }
  ...
  while ((m = refs.exec(s)) !== null) {
    ...
    if (m[2] !== undefined) {
      if (!m[2]) {
        if (labels.has(labelKey(m[1]))) protect(m.index, refs.lastIndex);

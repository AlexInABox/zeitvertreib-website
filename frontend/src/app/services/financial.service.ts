...
export class FinancialService {
    ...
    getTransactions(): Observable<FinancialTransaction[]> {
        return this.http.get<{transactions: FinancialTransaction[], total: number, limit: number, offset: number}>(`${this.apiUrl}/financial/transactions`, {
    ...
    getRecurringTransactions(): Observable<RecurringTransaction[]> {
        return this.http.get<{recurring_transactions: RecurringTransaction[], total: number}>(`${this.apiUrl}/financial/recurring`, {
            headers: this.getAuthHeaders()
        }).pipe(
            map((response: any) => response.recurring_transactions || [])
        );
    }
    ...
    createRecurringTransaction(transaction: Omit<RecurringTransaction, 'id' | 'created_at' | 'is_active'>): Observable<RecurringTransaction> {

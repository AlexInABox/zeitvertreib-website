...
- **Database Seed**: `npm run db:seed` wipes the local D1 and fills it with random demo data (players, fakeranks, moderation cases incl. related users and CedMod links; case _files_ live in S3 and are not seeded). Deterministic with `node devscripts/seed-dev-data.mjs --seed=42`.

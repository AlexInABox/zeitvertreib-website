...
import { validateSession, createResponse, fetchSteamUserData, fetchDiscordUserData, isTeam, validateSteamId, getSprayImage } from '../utils.js';
...
export async function getUserData(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  if (url.searchParams.has('discordid')) {
    ...
    const user = await db.select().from(playerdata)
  ...
  const playerdataResult = await db.select().from(playerdata)
  ...
  const fakerankResult = await db.select().from(fakeranks)
    .where(eq(fakeranks.userid, steamId))
    .limit(1);
  if (fakerankResult[0]) {
    fakerankData = {
  ...
  const fakerankBanResult = await db.select().from(fakerankBans)
    .where(eq(fakerankBans.userid, steamId))
    .limit(1);


  let sprayData: { id: number; name: string; full_res: string }[] = [];
  const spraysResult = await db.select().from(sprays)
    .where(eq(sprays.userid, steamId));
  if (spraysResult.length > 0) {
    sprayData = await Promise.all(spraysResult.map(async spray => ({
      id: spray.id,
      name: spray.name,
      full_res: (await getSprayImage(spray.id, env)) || ""
    })));
  }
  const sprayBanResult = await db.select().from(sprayBans)
    .where(eq(sprayBans.userid, steamId))
    .limit(1);


  let createdCasesData: { caseId: string; title: string; category: CaseCategory; createdAt: number }[] = [];
  const casesResult = await db.select().from(cases)
  ...
  const linkedCasesResult = await db.select().from(casesRelatedUsers)
    .where(
      eq(casesRelatedUsers.steamId, steamId));

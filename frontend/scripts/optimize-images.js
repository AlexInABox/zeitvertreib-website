...
(async () => {
  try {
    ...
    if (!found) {
      console.error(
        'No source image found. Place the original image at:',
        inputNames.map((n) => path.join('frontend/public/assets', n)).join(' or '),
      );
      process.exit(1);
    }
    ...
    await sharp(found).resize({ width: 1920, withoutEnlargement: true }).webp({ quality: 75 }).toFile(outWebp);

    await sharp(found).resize({ width: 1920, withoutEnlargement: true }).jpeg({ quality: 80 }).toFile(outJpg);

...
async function generateDiscordAuthUrl(clientId: string, redirectUri: string): Promise<string> {
...
async function exchangeCodeForToken(code: string, env: Env, redirectUri: string): Promise<string> {
    const data = {
...
export async function handleDiscordLogin(
    ...
    const authUrl = await generateDiscordAuthUrl(env.DISCORD_CLIENT_ID, redirectUri);
...
export async function handleDiscordCallback(
    ...
    if (!code) {
        return createResponse({ error: 'No authorization code provided' }, 400, origin);
    }
    ...
    try {
        ...
        const steamConnection = connections.find((conn: any) => conn.type === 'steam');
        ...
        return createResponse({ error: 'Discord authentication failed' }, 500, origin);
    }
}

...
function getMultiplierForStep(seed: number, step: number): number {
  ...
  const baseMultiplier = Math.pow(1.15 + variance * 0.05, step - 1);
...
async function sendResultToDiscord(
  ...
  try {
    ...
    const isBigWin = finalPayout >= initialWager * 5;
...
export async function handleChickenCrossPost(request: Request, env: Env, ctx?: ExecutionContext): Promise<Response> {
  ...
  if (intent === 'MOVE' && !seed) {
    ...
    const step = 1;
    const stepMultiplier = getMultiplierForStep(newSeed, step);
    const currentPayout = Math.floor(bet * stepMultiplier);
    const now = Date.now();

...
const withCLI = (fn) => {
  ...
  const cli = (...args) =>
    spawnSync(process.execPath, [path.join(__dirname, 'normalize-quotes.js'), ...args], { cwd: dir, encoding: 'utf8' });
  try {
    fn(dir, cli);
  } finally {
...
t('CLI stdout is exact and read-only; --write updates only the requested file', () =>
  withCLI((dir, cli) => {
...
t('CLI rejects invalid/missing options, extra files and I/O errors with exit 2', () =>
  withCLI((dir, cli) => {
    ...
    for (const args of [
      ...
      ['a.md', '--quotes', 'invalid'],
      ['a.md', '--reference'],
      ['a.md', '--reference', '--write'],
      ['a.md', '--reference', 'missing.md'],
      ['a.md', '--reference', 'a.md', '--quotes', 'curly'],
      ['a.md', '--reference', 'a.md', '--reference', 'a.md'],
      ['a.md', '--quotes', 'curly', '--bogus'],
      ['a.md', 'b.md', '--quotes', 'curly'],
      ['a.md', '--quotes', 'curly', '--quotes', 'straight'],
      ['missing.md', '--quotes', 'curly'],
      ['.', '--quotes', 'curly', '--write'],
    ]) {
      const r = cli(...args);
...
t('CLI defaults to auto and uses the original reference for preview and write', () =>
  withCLI((dir, cli) => {
...
t('a list fence ends when its container ends, including sibling items', () => {
  for (const suffix of ['"live"', '- "live"']) {
    ...
    assert.deepStrictEqual(
      check(source, { quotes: 'curly' }).hard.map((x) => x.line),
      [4],
    );
  }
  const source = '- ```\n  "code"\n\n  "still code"';
...
t('quoted URL punctuation leaves an enclosing prose parenthesis visible', () => {
  ...
  assert.deepStrictEqual(
    check("(e.g. see 'https://example.com/Foo_(bar).').", { latinAbbrev: 'parentheses' }).hard,
    [],
  );
});
...
t('inline destinations require a matching unescaped link-text opener in the same block', () => {
  for (const source of [
    'This ](url "actual prose")',
    '\\[x](url "actual prose")',
    '[x] text ](url "actual prose")',
    '[x\n\n](url "actual prose")',
  ]) {
    assert.strictEqual(curly(source), source.replace('"actual prose"', '“actual prose”'));
    assert.ok(check(source, { quotes: 'curly' }).hard.length);
...
t('explicit quotes bound bare URLs even when URL parentheses are unmatched', () => {
  ...
  for (const url of [
    ...
    "https://example.com/(O'Reilly)",
    "https://example.com/(unclosed/O'Reilly",
  ]) {
  ...
  for (const source of [
    "https://example.com/(O'Reilly",
    "<https://example.com/(O'Reilly>",
    '[docs](<https://example.com/(O\'Reilly> "title")',
  ]) {
    assert.strictEqual(curly(source + ' "live"'), source + ' \u201clive\u201d');
  }
});

 ...
 */
async function validateAdminAccess(request: Request, env: Env): Promise<{ isAdmin: boolean; session?: any; error?: string }> {
...
export async function handleGetTransactions(request: Request, env: Env): Promise<Response> {
    ...
    try {
        ...
        const result = await env['zeitvertreib-data'].prepare(query).bind(...params).all();

        // Transform database records to match frontend interface
        const transformedTransactions = result.results?.map((row: any) => ({
            id: row.id,
...
export async function handleGetRecurringTransactions(request: Request, env: Env): Promise<Response> {
    ...
    try {
        const result = await env['zeitvertreib-data'].prepare(`
    ...
    try {
        ...
        for (const recurring of recurringTransactions) {
            try {
                ...
                await env['zeitvertreib-data'].prepare(`
                ...
                await env['zeitvertreib-data'].prepare(`
                    ...
                    WHERE id = ?
                `).bind(nextExecution, today, recurring.id).run();
...
export async function handleDeleteTransaction(request: Request, env: Env): Promise<Response> {
    ...
    try {
        ...
        `).bind(parseInt(transactionId)).run();
...
export async function handleGetSummary(request: Request, env: Env): Promise<Response> {
    ...
    try {
        // Get total balance
        const balanceResult = await env['zeitvertreib-data'].prepare(`
            SELECT 
        ...
        const monthlyResult = await env['zeitvertreib-data'].prepare(`
            SELECT 
        ...
        const currentMonthResult = await env['zeitvertreib-data'].prepare(`
            SELECT 
                SUM(CASE WHEN transaction_type = 'income' THEN amount ELSE 0 END) as income,
                SUM(CASE WHEN transaction_type = 'expense' THEN amount ELSE 0 END) as expenses
            FROM financial_transactions
        ...
        const categoryResult = await env['zeitvertreib-data'].prepare(`
            SELECT 
                category,
                transaction_type,
                SUM(amount) as total,

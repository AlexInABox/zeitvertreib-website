...
function flatten(root) {
  ...
  if (!reference.startsWith('## What to remove or fix\n') || reference.split(boundary).length !== 2)
    throw new Error('Invalid pattern reference boundaries');
  ...
  for (const [name, content] of [
    ['catalog', reference.slice(0, index)],
    ['profiles', reference.slice(index)],
  ]) {
...
if (require.main === module) {
  ...
  if (process.argv.includes('--check')) {
    if (!fs.existsSync(target) || fs.readFileSync(target, 'utf8').replace(/\r\n/g, '\n') !== result)
      throw new Error('SKILL.full.md drifted; run node scripts/flatten-skill.js');

...
assert.equal(
  flatten(root),
  normalized(path.join(root, 'SKILL.full.md')),
  'Flattened artifact must equal canonical content',
);
...
const portableBody = (file) =>
  normalized(path.join(root, file))
    .replace(/^---\n[\s\S]*?\n---\n/, '')
...
try {
  ...
  fs.writeFileSync(
    path.join(fixture, 'references/patterns.md'),
    patterns.replace('## Context profiles', '## Missing profiles'),
  );
  assert.throws(() => flatten(fixture), /boundaries/);
  fs.writeFileSync(path.join(fixture, 'references/patterns.md'), patterns);
  for (const mutation of [
    front.replace('<!-- patterns:catalog -->', ''),
    front + '\n<!-- patterns:catalog -->\n\n',
  ...
  fs.writeFileSync(
    path.join(fixture, 'references/patterns.md'),
    patterns.replace('delve / delve into', 'MUTATED vocabulary'),
  );

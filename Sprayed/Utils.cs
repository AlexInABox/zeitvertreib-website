...
public static class Utils
{
    ...
    // Track if a fetch is already running to prevent concurrent fetches
    private static volatile bool _isFetching;

    // Multi-user fetch - fetch spray IDs and names for all players (fire-and-forget, runs on background thread)
    public static void SetSpraysForAllUsersFromBackendAsync()
    {
        if (_isFetching)
        {
            Logger.Debug("Spray fetch already in progress, skipping", Plugin.Instance.Config!.Debug);
            return;
        }
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                await SetSpraysForAllUsersFromBackendInternal();
            }
            catch (Exception ex)
            {
                Logger.Error($"Background spray fetch failed: {ex}");
            }
        });
    }

    // Internal implementation - runs on background thread
    private static async Task SetSpraysForAllUsersFromBackendInternal()
    {
        ...
        try
        {
            ...
            try
            {
                ...
                string useridsQuery =
                    string.Join("&", players.Select(p => $"userids={Uri.EscapeDataString(p.UserId)}"));
                ...
                Logger.Debug($"After cleanup - UserSprayIds keys: {string.Join(", ", UserSprayIds.Keys)}",
                    config.Debug);
                Logger.Debug($"After cleanup - UserSprayData keys: {string.Join(", ", UserSprayData.Keys)}",
                    config.Debug);
                Logger.Debug(
                ...
                string changedQuery =
                    string.Join("&", changedUserids.Select(u => $"userids={Uri.EscapeDataString(u)}"));
